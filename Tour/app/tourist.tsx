...
const GradientView = ({ colors, style, children }: GradientViewProps) => {
  return (
...
const IconComponent = ({ name, size = 24, color = "#000", style }: IconComponentProps) => {
  const icons = {
    "heart-pulse": "❤️",
    "gas-cylinder": "💨",
    battery: "🔋",
  ...
  return (
    <Text style={[{ fontSize: size, color }, style]}>{icons[name as keyof typeof icons] || "•"}</Text>
  );
};
...
const SafetyGauge = ({ score, colors }: SafetyGaugeProps) => {
...
const MetricCard = ({ icon, value, unit, label, color, colors, delay = 0 }: MetricCardProps) => {
  ...
  <IconComponent name={icon} size={20} color={colors.white} style={{}} />
      </View>
...
export const SmartTouristDashboard = () => {
  ...
  const touristData = {
    ...
    digital_id: {
      qr_code:
        "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=T-78901",
  ...
  const { latitude, longitude, address } = touristData.wearable_iot.last_location;
      ...
      <View style={[styles.card, { backgroundColor: colors.card }]}>
        ...
        <View style={styles.safetyDetails}>
          <View style={styles.safetyDetailItem}>
            <Text
              style={[
                styles.safetyDetailLabel,
                { color: colors.textSecondary },
              ]}
            >
              Location Status
            </Text>
            <Text style={[styles.safetyDetailValue, { color: colors.success }]}>
              Safe Zone
            </Text>
          </View>
          <View style={styles.safetyDetailItem}>
        ...
        {touristData.alerts.map((alert, index) => (
          ...
          >
            <View style={styles.alertContent}>
              ...
              <Text
                style={[styles.alertLocation, { color: colors.textTertiary }]}
              >
                {alert.location}
              </Text>
  ...
  const renderItinerary = () => (
    ...
    >
        ...
        <View style={styles.progressContainer}>
          <View style={styles.progressInfo}>
            <Text style={[styles.progressDay, { color: colors.text }]}>
              Day {touristData.trip_itinerary.current_day} of{" "}
              {touristData.trip_itinerary.total_days}
            </Text>
            <Text
              style={[styles.progressLocation, { color: colors.textSecondary }]}
            >
      ...
      >
  {/* Map Section - only keep the correct MapView block below */}
        <MapView
          style={styles.map}
          initialRegion={{
            latitude: touristData.wearable_iot.last_location.latitude,
            longitude: touristData.wearable_iot.last_location.longitude,
            latitudeDelta: 0.01,
            longitudeDelta: 0.01,
          }}>
          <Marker
            ...
            description={touristData.wearable_iot.last_location.address}
          />
          {currentLocation && (
            <Marker
        ...
        <View style={styles.infoBox}>
          <Text style={[styles.mapLocationText, { color: colors.text }]}>
            Last Known Location:{" "}
            {touristData.wearable_iot.last_location.address}
          </Text>
          <TouchableOpacity
            style={[styles.mapButton, { backgroundColor: colors.primary }]}
            onPress={() => {
              const { latitude, longitude } =
                touristData.wearable_iot.last_location;
    ...
    >
      ...
      <View style={[styles.card, { backgroundColor: colors.card }]}>
        ...
        <Text style={[styles.sosDescription, { color: colors.textSecondary }]}>
          This will immediately notify emergency services and your emergency
        ...
        {touristData.contact_info.emergency_contacts.map((contact, index) => (
          ...
          >
            ...
            <View style={styles.contactInfo}>
              ...
              <Text
                style={[styles.contactPhone, { color: colors.textTertiary }]}
              >
                {contact.phone}
              </Text>
            </View>
  ...
  return (
    ...
    >
      ...
      {/* Digital ID Modal */}
      <Modal
        visible={showDigitalID}
        animationType="slide"
        transparent={true}
        onRequestClose={() => setShowDigitalID(false)}
      >
          ...
          <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
            ...
            >
              ...
              <View style={styles.qrContainer}>
                <Image
                  source={{ uri: touristData.digital_id.qr_code }}
                  style={styles.qrCode}
                />
              </View>
              ...
              <View style={styles.idDetails}>
                <Text style={styles.idValidity}>
                  Valid:{" "}
                  {new Date(
                    touristData.digital_id.validity_start
                  ).toLocaleDateString()}{" "}
                  -{" "}
                  {new Date(
                    touristData.digital_id.validity_end
                  ).toLocaleDateString()}
                </Text>
                <View
                  style={[styles.idStatus, { backgroundColor: colors.success }]}
                >
                  <Text style={styles.idStatusText}>
                    ✓ {touristData.digital_id.status.toUpperCase()}
                  </Text>
                </View>
              </View>
            </GradientView>
            ...
            <Text
              style={[styles.blockchainInfo, { color: colors.textSecondary }]}
            >
